...
void config_time_base(alt_u32, SENSOR_OBJECT*);// Records the timebase
void init_measurement(SENSOR_OBJECT*);			//Initializes the queue
void read_accelerometerX(QUEUE *);
void update_graph(SENSOR_OBJECT*);
void draw_graph(SENSOR_OBJECT*);				//Draws the graph axes
...
unsigned int i2bcd(unsigned int i);//utility function for presenting time integer to bcd
void update_time(unsigned int i);  //Prints the time
